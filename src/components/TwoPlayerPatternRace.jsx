...
export default function TwoPlayerPatternRace({ p1CarId, p2CarId, onFinish, onBack }) {
  var p1Car = getCarById(p1CarId);
  var p2Car = getCarById(p2CarId);

  var problemsState = useState(function () {
    return generatePatternSet(TOTAL_PROBLEMS * 2);
  });
  ...
  var handleAnswer = useCallback(function (choiceItem) {
    ...
    var isCorrect = choiceItem.id === current.answer.id;
    setRevealed(true);
  ...
  return (
    <div className="screen math-race-screen pattern-race-screen two-player-race">
